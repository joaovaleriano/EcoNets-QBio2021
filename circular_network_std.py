...
def coop_freq_curves(n_nodes, init_coop_freq, n_neighbs, nt, eps, seeds):
    # n: number of nodes in the network
    # init_coop_freq: cooperator frequency in the initial condition
    # n_neighb: number of neighbors for each node
    # nt: number of timesteps to run time evolution
    ...
    # List with different seeds
    seeds = [1, 25, 566, 94, 104]
    ...
    for j in tqdm(range(len(b))):
        ...
        for k in range(len(seeds)):
            ...
            coop_freqs[0,k] = 1 - sum(nx.get_node_attributes(network, "strat").values()) / n_nodes
            ...
            mean_coop_freq = np.mean(coop_freqs, axis=2)
            std_coop_freq = np.std(coop_freqs, axis=2)
            '''print(np.shape(std_coop_freq))
            print(type(std_coop_freq))'''

            
    # Plot cooperator frequency time evolution for different b values
    
    for i in range(len(b)):
        plt.plot(timesteps, mean_coop_freq[:,i], label=f"$b = {b[i]:0.2f}$") # Plot cooperator frequency over time
        plt.fill_between(timesteps, std_coop_freq[:,i])
