...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 20 16:47:34 2021
...
def gen_net(number_of_nodes, knn, rewire, coop_freq, seed):
    ...
    network = nx.watts_strogatz_graph(number_of_nodes, knn, rewire, seed)
...
def show_time_evol(n_nodes, init_coop_freq, knn, rewire, nt, b, eps, seed=None):
    ...
    network, colormap = gen_net(n_nodes, knn, rewire, init_coop_freq, seed)
    calc_fit_mat(network, payoff_mat)
...
def gen_coop_freq_evol(n_nodes, nt, b, eps, seeds, init_coop_freq, knn, rewire):
    ...
    for j in tqdm(range(len(b))):
        ...
        for k in range(len(seeds)):
            ...
            network, colormap = gen_net(n_nodes, knn, rewire, init_coop_freq, seeds[k])
            calc_fit_mat(network, payoff_mat)
...
def plot_coop_freq_evol(coop_freqs, b, title=None, save_files=False):
    ...
    for i in range(len(b)):
        plt.plot(timesteps, avg_coop_freqs[:,i], color=colors[i], lw=3,
                 label=f"$b = {b[i]:0.2f}$", alpha=1.) # Plot cooperator frequency over time
...
#                                 init_coop_freq=0.9, knn=8, rewire=0.0)
# plot_coop_freq_evol(coop_freqs, b, title="Rewire Probability = 0%", save_files=False)
...
# coop_freq = gen_final_coop_freq(n_nodes=100, knn=8, rewire=0.1, nt=80, nt_save=20, 
...
def gen_final_coop_freq(n_nodes, knn, rewire, nt, nt_save, b, eps=0., init_coop_freq=0.5, seeds=[None]):
    ...
    # Array to store cooperator frequency for different b values and different timesteps
    coop_freq = np.zeros((len(b), len(rewire), len(seeds), nt_save))
    
    # Loop over b values
    for j in range(len(b)):
        for k in range(len(rewire)):
            for s in range(len(seeds)):
                payoff_mat = np.array([[1., 0],[b[j], eps]]) # Define the payoff matrix
                
                network, colormap = gen_net(n_nodes, knn, rewire[k], init_coop_freq, seeds[s])
                calc_fit_mat(network, payoff_mat)
...
# Plot statistics of final cooperator frequency for different b values
def plot_final_coop_freq(coop_freq, b, rewire, save_files=False):
    # coop_freq: array containing some timesteps of the cooperator frequency for different values of b
    #        |-> shape: (len(b), # of timesteps)
    # b: array of b values considered for generating "coop_freq"
    # save_files: wether or not to save plot to file
    ...
    # Generate errorbars from minimum to maximum cooperator frequencies
    errorbars = np.zeros((2, len(b), len(rewire)))
    for j in range(len(rewire)):
        for i in range(len(b)):
    ...
    for j in range(len(b)):
        # Plot markers with errorbars
        plt.errorbar(rewire, final_coop_freq_avg[j,:], errorbars[:,j,:], 
...
rewire = np.linspace(0, 1, 5)
coop_freq = gen_final_coop_freq(n_nodes=100, knn=8, rewire=rewire, nt=80, nt_save=20, 
                                b=b, eps=0., init_coop_freq=0.9, seeds=[i for i in range(20)])

plot_final_coop_freq(coop_freq, b, rewire, save_files=False)
