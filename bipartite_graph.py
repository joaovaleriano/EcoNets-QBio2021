...
def gen_lat_net(n0, n1, coop_freq0, coop_freq1, seed=None):
    # number_of_nodes: number of nodes in the network
    # knn: number of nearest neighbors to connect
    # rewire: probability of rewiring a connection
    # coop_freq: cooperator frequency
    ...
    # Create network
    A = nx.grid_2d_graph(n0, n0, periodic=True)
    ...
    B = nx.grid_2d_graph(n1, n1, periodic=True)
    B = nx.relabel.relabel_nodes(B, map)
    ...
    # Generate array with strategies for each node, randomly sorted
    np.random.seed(seed)
    # strat0 = np.random.choice([0,1], (n0, n0), p=[coop_freq0, 1-coop_freq0])
    # strat1 = np.random.choice([0,1], (n1, n1), p=[coop_freq1, 1-coop_freq1])
    
    strat0 = np.zeros((n0,n0))
    strat1 = np.zeros((n1,n1))
    
    strat0[n0//2-2:n0//2+1, n0//2-2:n0//2+1] = 1.
    strat1[n1//2-2:n1//2+1, n1//2-2:n1//2+1] = 1.
    ...
    network = nx.compose(A, B)
    network.remove_edges_from(network.edges())
    
    for i in range(n0):
        for j in range(n0):
            network.add_edge((i,j), ((i-1)%n0+n0, (j-1)%n0+n0))
            network.add_edge((i,j), ((i-1)%n0+n0, j%n0+n0))
            network.add_edge((i,j), ((i-1)%n0+n0, (j+1)%n0+n0))
            network.add_edge((i,j), (i%n0+n0, (j-1)%n0+n0))
            network.add_edge((i,j), (i%n0+n0, (j+1)%n0+n0))
            network.add_edge((i,j), ((i+1)%n0+n0, (j-1)%n0+n0))
            network.add_edge((i,j), ((i+1)%n0+n0, j%n0+n0))
            network.add_edge((i,j), ((i+1)%n0+n0, (j+1)%n0+n0))
    ...
    for i in range(n0):
        for j in range(n0):
            ...
            if strat0[i,j] == 0: # Set node as a cooperator
                network.nodes[i,j]["strat"] = 0
                colormap.append("blue")
            
            else: # Set node as a defector
                network.nodes[i,j]["strat"] = 1
                colormap.append("yellow")
                
            # Initialize the fitness of each node
            network.nodes[i,j]["fit"] = 0
            
            # Set node positions
            # network.nodes[i,j]["pos"] = (0,-i)
        
    for i in range(n0, n0+n1):
        for j in range(n0, n0+n1):
        
            if strat1[i-n0, j-n0] == 0: # Set node as a cooperator
                network.nodes[i,j]["strat"] = 0
                colormap.append("blue")
...
# Calculate fitness matrix over the network
def calc_fit_mat_lat(network, n0, n1, payoff_mat0, payoff_mat1):
    # network: the network object from the NetworkX package
    # payoff_mat: payoff matrix for the game
    ...
    for i in range(n0):
        for j in range(n0):
            ...
            network.nodes[i,j]["fit"] = 0 # Set fitness to zero initially
            
            # Sum the contribution of each neighbor for the fitness of the focal node
            for nb_i, nb_j in network.neighbors((i,j)):
                # print(network.nodes[nb]["strat"])
                network.nodes[i,j]["fit"] += payoff_mat0[network.nodes[i,j]["strat"],
                                                    network.nodes[nb_i, nb_j]["strat"]]
        # print("\n")
    # Loop over nodes
    for i in range(n0, n0+n1):
        for j in range(n0, n0+n1):
            ...
            # Sum the contribution of each neighbor for the fitness of the focal node
            for nb_i, nb_j in network.neighbors((i,j)):
                network.nodes[i,j]["fit"] += payoff_mat1[network.nodes[i,j]["strat"],
                                                        network.nodes[nb_i, nb_j]["strat"]]
...
# Evolution of the network by a time step
def evolve_strats_lat(network, colormap, n0, n1, payoff_mat0, payoff_mat1):
    # network: the network object from the NetworkX package
    # colormap: colors of the nodes indicating cooperators and defectors
    # payoff_mat: payoff matrix for the game
    ...
    A = bipartite.projected_graph(network, list(network.nodes)[:n0*n0])
    B = bipartite.projected_graph(network, list(network.nodes)[n0*n0:])
    ...
    for i in range(n0):
        for j in range(n0):
            ...
            # Check if focal node is cooperator or defector and add its fitness to
            # the corresponding list
            if A.nodes[i,j]["strat"] == 0:
                coop_nb_fit.append(A.nodes[i,j]["fit"])
            else:
                defec_nb_fit.append(A.nodes[i,j]["fit"])
            
            # Loop over neighbors, adding their fitnesses to the appropriate lists
            for nb_i, nb_j in A.neighbors((i,j)):
                if A.nodes[nb_i, nb_j]["strat"] == 0:
                    coop_nb_fit.append(A.nodes[nb_i, nb_j]["fit"])
                else:
                    defec_nb_fit.append(A.nodes[nb_i, nb_j]["fit"])
                    
            # Check if cooperators or defectors neighbors have higher fitness and
            # update the focal node's strategy
            if max(coop_nb_fit) > max(defec_nb_fit):
                network.nodes[i,j]["strat"] = 0
                colormap[i*n0+j] = colors[0]
                coopA += 1
            
            elif max(coop_nb_fit) < max(defec_nb_fit):
                network.nodes[i,j]["strat"] = 1
                colormap[i*n0+j] = colors[1]
        ...
        # new strategy for the focal node
            ...
            tie_strats = [0]*n_coop_tie + [1]*n_defec_tie
            sort_strat = np.random.choice(tie_strats)
            network.nodes[i,j]["strat"] = sort_strat
            colormap[i*n0+j] = colors[sort_strat]
    ...
    for i in range(n0, n0+n1):
        for j in range(n0, n0+n1):
            ...
            # Check if focal node is cooperator or defector and add its fitness to
            # the corresponding list
            if B.nodes[i,j]["strat"] == 0:
                coop_nb_fit.append(B.nodes[i,j]["fit"])
            else:
                defec_nb_fit.append(B.nodes[i,j]["fit"])
            
            # Loop over neighbors, adding their fitnesses to the appropriate lists
            for nb_i, nb_j in B.neighbors((i,j)):
                if B.nodes[nb_i, nb_j]["strat"] == 0:
                    coop_nb_fit.append(B.nodes[nb_i, nb_j]["fit"])
                else:
                    defec_nb_fit.append(B.nodes[nb_i, nb_j]["fit"])
                    
            # Check if cooperators or defectors neighbors have higher fitness and
            # update the focal node's strategy
            if max(coop_nb_fit) > max(defec_nb_fit):
                network.nodes[i,j]["strat"] = 0
                colormap[n0*n0+(i-n0)*n1+j-n0] = colors[0]
                coopB += 1
            
            elif max(coop_nb_fit) < max(defec_nb_fit):
                network.nodes[i,j]["strat"] = 1
                colormap[n0*n0+(i-n0)*n1+j-n0] = colors[1]
            ...
            # new strategy for the focal node
                ...
                sort_strat = np.random.choice(tie_strats)
                network.nodes[i,j]["strat"] = sort_strat
                colormap[n0*n0+(i-n0)*n1+j-n0] = colors[sort_strat]
                
                if sort_strat == 0:
                    coopB += 1
    
    # Calculate the new fitness matrix
    calc_fit_mat_lat(network, n0, n1, payoff_mat0, payoff_mat1)
    
    return coopA/(n0*n0), coopB/(n1*n1)
...
def gen_coop_freq_evol_lat(n0, n1, nt, b0, b1, seeds, init_coop_freq0, init_coop_freq1):
    # n: number of nodes in the network
    # nt: number of timesteps to run time evolution
    # b0: b parameter of payoff matrix A
    # b1: array for values of b parameter of payoff matrix A
    ...
    for j in tqdm(range(len(b1))):
        ...
        for s in range(len(seeds)):
            ...
            # Initialize network and calculate its fitness matrix
            network, colormap = gen_lat_net(n0, n1, init_coop_freq0, init_coop_freq1, 
                                            seed=seeds[s])
            A = bipartite.projected_graph(network, list(network.nodes)[:n0*n0])
            B = bipartite.projected_graph(network, list(network.nodes)[n0*n0:])
            ...
            coop_freqs0[0,j,s] = 1 - sum(nx.get_node_attributes(A, "strat").values()) / (n0*n0)
            coop_freqs1[0,j,s] = 1 - sum(nx.get_node_attributes(B, "strat").values()) / (n1*n1)
            
            # Time evolution of the network
            for i in range(1, nt):
                
                coop_freqs0[i,j,s], coop_freqs1[i,j,s] = evolve_strats_lat(network, colormap, n0, n1, payoff_mat0, payoff_mat1) # Evolve the network by a timestep
...
coop_freqs0, coop_freqs1 = gen_coop_freq_evol_lat(n0, n1, nt, b0, b1, seeds, 
                                              init_coop_freq0, init_coop_freq1)
plot_coop_freq_evol(coop_freqs0, coop_freqs1, b0, b1)
