...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 19 17:19:58 2021
...
def show_time_evol(n_nodes, init_coop_freq, n_edges, nt, b, eps, seed=None):
    ...
    network, colormap = gen_net(n_nodes, n_edges, init_coop_freq, seed)
    calc_fit_mat(network, payoff_mat)
...
def gen_coop_freq_evol(n_nodes, nt, b, eps, seeds, init_coop_freq, n_edges):
    ...
    for j in tqdm(range(len(b))):
        ...
        for k in range(len(seeds)):
            ...
            network, colormap = gen_net(n_nodes, n_edges, init_coop_freq, seeds[k])
            calc_fit_mat(network, payoff_mat)
...
coop_freqs = gen_coop_freq_evol(n_nodes=100, nt=100, b=b, 
                                eps=0., seeds=seeds, init_coop_freq=0.9, n_edges=150)
plot_coop_freq_evol(coop_freqs, b, save_files=True)
